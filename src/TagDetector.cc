...
std::vector<TagDetection> TagDetector::extractTags(const cv::Mat& image) {
   ...
   */
...
#ifdef APRILTAGS_SHOW_TIMING
            std::cout << "- Filtering took: " << getCurrentTime()-s_filter_start << " ms" << std::endl;
#endif
...
#ifdef APRILTAGS_USE_OPENCV
    {
        ...
        cv::multiply(dstMag, dstMag, dstMag); // we probably dont really need that if we make sure the thresholds hereafter are sqrted
    }
#else
    ...
    { // limit scope of storage
        ...
        for (int tr=0;tr<(int)numThreads;tr++)
        {
            ...
            std::inplace_merge(edges.begin(), edges.begin() + oldEdgeSize, edges.begin() + totalEdges);
        }
