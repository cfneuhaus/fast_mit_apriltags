...
public:
    ...
    //! Given that the whole quad spans from (0,0) to (1,1) in "quad space", compute the pixel
    //! coordinates for a given point within that quad.
    /*!  Note that for most of the Quad's existence, we will not know the correct orientation of the
     * tag. */
     ...
     */
    static void search(const FloatImage& fImage, std::vector<Segment*>& path, Segment& parent,
        int depth, std::vector<Quad>& quads, const std::pair<float, float>& opticalCenter);
