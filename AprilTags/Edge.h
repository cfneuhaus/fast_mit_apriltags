...
public:
    ...
    static float const
        maxEdgeCost; //!< 30 degrees = maximum acceptable difference in local orientations
    ...
    explicit Edge(int idxa, int idxb, int c)
        : pixelIdxA(idxa)
        , pixelIdxB(idxb)
        , cost(c)
    {
    ...
    //! Variant of calcEdges that uses push back on the vector instead of a manually kept fill count
    static void calcEdges2(float theta0, int x, int y, const FloatImage& theta,
        const FloatImage& mag, std::vector<Edge>& edges);

    //! Process edges in order of increasing cost, merging clusters if we can do so without
    //! exceeding the thetaThresh.
    static void mergeEdges(std::vector<Edge>& edges, UnionFindSimple& uf, float tmin[],
